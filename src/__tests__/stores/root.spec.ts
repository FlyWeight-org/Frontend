...
describe('request refresh-and-retry', () => {
  ...
  it('refreshes only once when several requests hit a 401 concurrently', async () => {
    ...
    const [a, b] = await Promise.all([
      ...
      requestJSON({ path: '/pilot/flights' }),
    ])
  ...
  it('drops the dead session and loads public content anonymously', async () => {
    const fetchMock = vi.fn<FetchFn>((url, init) => {
      ...
      return Promise.resolve(jsonResponse(200, { uuid: 'f2d', can_edit: false }))
    })
    vi.stubGlobal('fetch', fetchMock)

    const auth = useAuthStore()
    const result = await requestJSON<{ uuid: string }>({ path: '/flights/f2d.json' })

    expect(result.ok).toBe(true)
    if (result.ok) expect(result.val.body).toEqual({ uuid: 'f2d', can_edit: false })
