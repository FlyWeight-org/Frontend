...
export type LoadJSONUp = Omit<
  Load,
  "slug" | "bagsWeight" | "covid19Vaccination"
> & {
  bags_weight: number;
  covid19_vaccination: boolean;
};
...
export type FlightJSONDown = Omit<
  Flight,
  "UUID" | "date" | "loads" | "canEdit"
> & {
  uuid: string;
  date: string;
...
export type FlightJSONUp = Omit<
  Flight,
  "UUID" | "loads" | "date" | "canEdit"
> & {
  uuid: string;
  date: string;
};
...
export function editableFlightToJSON(
  flight: EditableFlight
): Partial<FlightJSONUp> {
  return {
    ...omit(flight, "date"),
    date: flight.date?.toJSON(),
  };
}

/** The shape of flight list item data sent from the back-end. */
export type FlightListItemJSONDown = Omit<
  FlightListItem,
  "passengerCount" | "date" | "UUID"
> & {
  uuid: string;
  date: string;
...
export function flightListItemFromJSON(
  JSON: FlightListItemJSONDown
): FlightListItem {
  return {
